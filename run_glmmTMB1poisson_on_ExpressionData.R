...
for(i in 1:ncol(data1)) {       # for-loop over columns
  ...
  model = try(glmmTMB(Expression ~ History*Environment*Selection+(1|AP:History),data=data3,family="poisson"))
  Anovamodel = try(as.data.frame(Anova(model, type='III')))
  if (class(Anovamodel) == "try-error") {
    ...
    Anovamodel <- data.frame(Chisq, Df, pvalue)
  }
  Anovamodel[nrow(Anovamodel) + 1,] = c('NA', 'NA',upOrDownPT)
  Anovamodel[nrow(Anovamodel) + 1,] = c('NA', 'NA',upOrDownNL)
  Anovamodel[nrow(Anovamodel) + 1,] = c('NA', 'NA',upOrDownWPT)
  Anovamodel[nrow(Anovamodel) + 1,] = c('NA', 'NA',upOrDownWNL)
  
  # changing row names of data frame
  rownames(Anovamodel)[9] <- "upOrDownPT"
  rownames(Anovamodel)[10] <- "upOrDownNL"
  rownames(Anovamodel)[11] <- "upOrDownWPT"
  rownames(Anovamodel)[12] <- "upOrDownWNL"
  
  #Sort Using Character row.names
  #subset
  subsetted <- as.data.frame(subset(Anovamodel, select = -c(`Chisq`,Df)))
  #change column name
  colnames(subsetted) <- c(geneName)
  #transpose
  transposed_of_model <- as.data.frame(t(subsetted),stringsAsFactors = FALSE)
  try(write.table(transposed_of_model, args[2] , append = TRUE, sep = '\t', col.names = FALSE, row.names = TRUE))
